...
sys.path.append(r'C:\WorkSpace\KDT\TORCH_TEXT\D1010\cgi-bin')
...
# 미리 선언하고 가는 것
vocab_df_me = pd.read_csv(r'C:\WorkSpace\KDT\TORCH_TEXT\D1010\cgi-bin\nose_vocab.csv', encoding='utf-8')
...
LABEL_TRANSLATE ={0:'다른 병원', 1:"이비인후과"}
...
# (2) 모델 로딩
if SCRIPT_MODE:
    pklfile_me = r'C:\WorkSpace\KDT\TORCH_TEXT\D1010\cgi-bin\model_train_all_2_0.9879336349924586.pth'
...
lstm_model_me = SentenceClassifier(n_vocab=n_vocab_me, hidden_dim=hidden_dim, embedding_dim=embedding_dim, n_layers=n_layers)   
...
msg ="병원"
if text != "":
    text_me = transform(text, vocab_me, type_='others')

    pred_me = rnn.predict_web(lstm_model_me, text_me, type_='others')
