    ...
    with torch.no_grad(): # 가중치 업데이트 없이 테스트 진행
        for X_batch, y_batch in test_DataLoader:
            ...
            if model_type == 'regression': # 회귀일 때
                ...
                score_test = r2_score(pred_test_y.cpu(), y_batch.cpu())  # GPU에서 연산 후 CPU로 이동
            elif model_type == 'binary': # 이진 분류일 때
                loss_test = F.binary_cross_entropy(pred_test_y, y_batch)
                score_test = f1_score(pred_test_y.cpu(), y_batch.cpu(), task='binary')
                ...
                score_test = f1_score(pred_test_labels.cpu(), y_batch1D.cpu(),  # CPU로 이동 후 F1 점수 계산
...
def training(train_DataLoader, test_DataLoader, model, model_type, optimizer,
    ...
    for i in range(1, epoch+1):  # 에포크 반복
        total_loss_train = 0  # 한 에포크당 합산할 손실값
        total_score_train = 0  # 한 에포크당 합산할 점수
        ...
        for X_batch, y_batch in train_DataLoader:
            ...
            if model_type == 'regression':  # 회귀일 때
                ...
                score_train = r2_score(pred_train_y.cpu(), y_batch.cpu())  # GPU에서 연산 후 CPU로 이동
            elif model_type == 'binary':  # 이진 분류일 때
                loss_train = F.binary_cross_entropy(pred_train_y, y_batch)
                score_train = f1_score(pred_train_y.cpu(), y_batch.cpu(), task='binary')  # CPU로 이동 후 계산
            elif model_type == 'multiclass':  # 다중 분류일 때
                y_batch1D = y_batch.reshape(-1)  # 1차원으로 변환
                loss_train = F.cross_entropy(pred_train_y, y_batch1D.long())
                pred_train_labels = torch.argmax(pred_train_y, dim=1)
                score_train = f1_score(pred_train_labels.cpu(), y_batch1D.cpu(),  # CPU로 이동 후 F1 점수 계산
                                       task='multiclass', num_classes=num_classes)
        ...
        if len(loss_test_history) == 1:  # 첫 에포크일 때
